...
final formProvider = ChangeNotifierProvider<FormProvider>((_) => FormProvider(_));
...
class _FormWidgetState extends ConsumerState<FormWidget> {
  ...
  @override
  void initState() {
    themeData = loadThemeData(widget.theme, widget.themeName ?? "widget_form", () => const FormWidgetThemeData())!;

    ref.read(formProvider).init(0, widget.steps.where((element) => element.goToNextStep).toList().length + (widget.showResumed ? 1 : 0));
    ref.read(formProvider).initData(
          title: widget.title ?? "",
          steps: widget.steps,
        );
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Expanded(
                  child: Opacity(
                    opacity: ref.watch(formProvider).page < ref.read(formProvider).maxLength ? 1 : 0,
                    child: InkWell(
                      onTap: () {
                        if (widget.onTapLater != null) widget.onTapLater!(null);
                      },
                      child: Padding(
                        padding: EdgeInsets.symmetric(vertical: formatHeight(6)),
                        child: Text(
                          "Plus tard",
                          textAlign: TextAlign.end,
                          style: widget.laterStyle ??
          ...
          children: [
            ...
            if (widget.showResumed)
              FormResumed(
                ...
                onTapModify: (index, step) => ref.read(formProvider).jumpToPage(index, step as int),
        ...
        children: [
                  ...
                  (e) => Column(
                    ...
                    children: [
                      FormGenerator.generateField(
                        context,
                        e,
                        onChanged: ((p0) => ref.read(formProvider).updateData(step.tag, e.tag, p0)),
                      ),
                      sh(8),
                    ],
                  ),
          ...
          if (widget.steps.last != step || widget.showResumed) ...[
            ...
            CTA.primary(
              ...
              onTap: () {
                if (step.key?.currentState?.validate() ?? true) {
                  if (widget.onSubmit != null) widget.onSubmit!(ref.read(formProvider).data);
                }
              },
            ),
          ],
